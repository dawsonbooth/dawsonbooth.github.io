// https://github.com/Noitidart/valtio-persist

import { get, isPlainObject, omit, set, throttle, toPath } from 'lodash-es'
...
const STORAGE_ENGINE: ProxyPersistStorageEngine = {
...
const persistence = {
  ...
  getStorage: () => STORAGE_ENGINE,
  onBeforeBulkWrite: throttle(bulkWrite => bulkWrite(), 1000),
}

type DottedPath<T, K extends keyof T> = K extends string
  ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
    T[K] extends Record<string, any>
    ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
      T[K] extends ArrayLike<any>
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        K | `${K}.${DottedPath<T[K], Exclude<keyof T[K], keyof any[]>>}`
      : K | `${K}.${DottedPath<T[K], keyof T[K]>}`
    : K
  : never

type DottedAccessor<T> = DottedPath<T, keyof T> | keyof T

const reset = <T extends object>(store: T, initial: T) => {
  Object.assign(store, initial)
}

const proxyWithPersist = <S extends object>({
  name,
  initialState,
  migrations = 0,
  whitelist,
}: {
  name: IProxyWithPersistInputs<S>['name']
  initialState: S
  migrations?: number | IProxyWithPersistInputs<S>['migrations']
  whitelist?: DottedAccessor<S>[]
}) => {
  const inputs: IProxyWithPersistInputs<S> = {
    name,
    initialState,
    getStorage: () => STORAGE_ENGINE,
    onBeforeBulkWrite: throttle(bulkWrite => bulkWrite(), 1000),
    ...(typeof migrations === 'number'
      ? {
          version: migrations,
          migrations: Array.from({ length: migrations }, (_, i) => i + 1).reduce(
            (record, key) => ({ ...record, [key]: async () => reset(proxyObject, initialState) }),
            {},
          ),
        }
      : { version: Object.keys(migrations).length, migrations }),
    persistStrategies:
      whitelist?.reduce(
        (record, key) => ({ ...record, [key]: persistence.persistStrategies }),
        {},
    ...
    await Promise.all(
      Object.entries(
        ...
        const proxySubObject =
          isPersistingMainObject || pathStart === '' ? proxyObject : get(proxyObject, pathStart)
